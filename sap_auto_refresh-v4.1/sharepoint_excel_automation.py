    ...
    while time.time() < deadline:
        try:
            ...
            if new_names:
                ...
                # 锚定工作簿始终存在，COM 连接已稳定，等待 3s 确保加载完成
                log(f"  工作簿已检测到，等待 3s 确保加载完成...", "WAIT")
                time.sleep(3)

                # ── COM 连接稳定性验证 ────────────────────────
                # 首次打开文件时 SAP 插件初始化可能导致 COM 暂时不稳定，
                # 循环验证直到 workbook.Application.Ready 可正常访问
                wb_obj = None
                ...
                if wb_obj is not None:
                    ...
                    for check_i in range(5):
                        try:
                            ...
                            log(f"  COM 连接验证第 {check_i+1} 次未通过，等待 3s...", "WARN")
                            time.sleep(3)
                            # 重新获取对象
                            try:
                                pythoncom.CoInitialize()
...
def run_sap_analysis_refresh(workbook, is_first_file):
    ...
    # ── 首文件 COM 连接稳定性保护 ─────────────────────────
    # 第一个文件通过 ms-excel 协议打开后，SAP Analysis 插件初始化
    # 可能导致 COM 对象短暂断连（"对象没有连接到服务器"），
    # 此处对首文件进行重试机制，确保 COM 连接稳定后再执行操作。
    MAX_RETRIES = 2   # 锚定工作簿保持 COM 稳定，最多重试 2 次即可
    RETRY_WAIT  = 8   # 每次重试前等待秒数

    for attempt in range(1, MAX_RETRIES + 1):
        try:
            # 每次重试前重新获取工作簿对象（刷新 COM 连接）
            if attempt > 1:
                log(f"  [重试 {attempt}/{MAX_RETRIES}] 等待 {RETRY_WAIT}s 后重新获取 COM 对象...", "WARN")
                time.sleep(RETRY_WAIT)
                pythoncom.CoInitialize()
                fresh_wb = get_fresh_workbook(workbook.Name)
                if fresh_wb is not None:
                    workbook = fresh_wb
                    log(f"  [重试 {attempt}/{MAX_RETRIES}] COM 对象已刷新", "OK")
                else:
                    log(f"  [重试 {attempt}/{MAX_RETRIES}] 无法获取工作簿对象，继续尝试...", "WARN")
                    continue
            ...
            if is_com_error and attempt < MAX_RETRIES:
                log(f"  [COM 错误] {e}", "WARN")
                log(f"  首文件 COM 连接不稳定，将进行第 {attempt+1} 次重试...", "WARN")
                continue
            else:
                log(f"  处理流程出错: {e}", "ERROR")
                return False

    log(f"  首文件重试 {MAX_RETRIES} 次后仍失败", "ERROR")
    return False
...
def close_workbook_safe(workbook):
    """关闭工作簿（已 Save 过，此处 SaveChanges=False 避免弹窗）。
    锚定工作簿（用户预先打开的）不会被关闭。"""
    try:
        name = workbook.Name
        # 保护锚定工作簿：不关闭用户预先打开的 Excel
...
def process_file_queue(file_queue):
    ...
    先开后关规则：
      Open(File_1) → 处理 → Save
      Open(File_2) → 处理 → Save → Close(File_1)
      Open(File_3) → 处理 → Save → Close(File_2)
      ...
      Close(最后一个文件)
    """
    if not file_queue:
    ...
    current_wb = None
    is_first   = False  # 锚定工作簿已稳定 COM 连接，无需首文件特殊处理
    success_count = 0
    fail_count    = 0
    ...
    for i, file_info in enumerate(file_queue):
        ...
        if current_wb is not None:
            ...
            close_workbook_safe(current_wb)
