...
REFRESH_TIMEOUT      = 300         # SAP 刷新最大等待时间（秒）
...
def click_analysis_workbook_prompt(workbook):
    ...
    try:
        ...
        # 第三步：点击"工作簿提示"菜单项（增加超时，避免无限等待）
        try:
            # 使用 wait 方法设置超时 5 秒，若未出现则返回 False
            wb_prompt = win.child_window(title="工作簿提示", control_type="MenuItem").wait('exists', timeout=5)
            if wb_prompt:
                ...
                log("  工作簿提示菜单项未出现", "WARN")
                return False
        except Exception as e:
            log(f"  工作簿提示菜单项查找失败: {e}", "ERROR")
            return False
    ...
    while True:
        ...
        if fetch_detected and not fetch_disappeared:
            ...
            if elapsed > fetch_timeout:
                log(f"  [超时] 服务器数据获取超过 {fetch_timeout}s", "ERROR")
                result["fetch_info"]["result"] = "TIMEOUT"
        ...
        # 情况 2: 获取数据弹窗消失，但确认按钮还没出现 → 标记消失，继续等
        if fetch_detected and not found_fetch and not fetch_disappeared:
            fetch_disappeared = True
            fetch_wait = round(time.time() - fetch_detect_time, 1)
            log(f"  数据获取弹窗已消失（耗时 {fetch_wait}s），等待确认对话框...", "INFO")
            result["fetch_info"] = {"detected": True, "wait_seconds": fetch_wait, "result": "COMPLETED"}
...
# ────────────────────────────────────────────────────────────
#  COM 连接辅助函数（新增，用于彻底刷新）
# ────────────────────────────────────────────────────────────

def get_fresh_excel_application():
    """重新初始化 COM 并获取 Excel.Application 对象"""
    try:
        pythoncom.CoInitialize()
        return win32com.client.GetActiveObject("Excel.Application")
    except Exception as e:
        log(f"  获取 Excel Application 失败: {e}", "WARN")
        return None


# ────────────────────────────────────────────────────────────
#  核心 SAP 刷新逻辑（带 COM 重试）
# ────────────────────────────────────────────────────────────

def run_sap_analysis_refresh(workbook, is_first_file):
    """
    根据 TEST_MODE 选择执行模式：
      TEST_MODE=True  : 写入 A1 时间戳（验证流程）
      TEST_MODE=False : SAP Analysis 数据刷新（含完成检测）

    增强重试机制：
      - 所有文件均支持 COM 错误自动重试（不仅限首文件）
      - 每次重试前重新获取 Application 和工作簿对象，彻底刷新 COM 连接
      - 重试前进行深层 COM 稳定性验证
    """
    MAX_RETRIES = 3   # 最大重试次数
    RETRY_WAIT  = 15  # 每次重试前等待秒数（给 SAP 插件充足初始化时间）
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            # 每次重试（包括第一次）确保 COM 对象有效
            if attempt > 1:
                log(f"  [重试 {attempt}/{MAX_RETRIES}] 重新获取 Excel Application 和工作簿...", "WARN")
                time.sleep(RETRY_WAIT)

                # 重新初始化 COM
                pythoncom.CoInitialize()
                excel_app = get_fresh_excel_application()
                if excel_app is None:
                    log(f"  无法获取 Excel Application，继续等待...", "WARN")
                    time.sleep(5)
                    continue
                ...
                for wb in excel_app.Workbooks:
                    if wb.Name == wb_name:
                        ...
                        break
                if fresh_wb is None:
                    log(f"  未找到工作簿 [{wb_name}]，继续等待...", "WARN")
                    time.sleep(5)
                    continue
                ...
                try:
                    ...
                    _ = fresh_wb.Sheets(1).Range("A1").Value
                    workbook = fresh_wb
                    log(f"  COM 对象已刷新并验证通过", "OK")
                except Exception as ve:
                    log(f"  COM 验证失败: {ve}", "WARN")
                    continue
            else:
                # 第一次尝试，简单验证
                try:
                    ...
                    log(f"  首次 COM 验证失败: {e}，尝试刷新...", "WARN")
                    # 尝试重新获取一次
                    fresh_wb = get_fresh_workbook(wb_name)
            ...
            # 执行实际刷新操作
            return _do_sap_refresh(workbook, is_first_file)
...
def _do_sap_refresh(workbook, is_first_file):
    ...
    try:
        ...
        # ════════════════════════════════════════════════════════
            ...
            # ── 操作前最终 COM 验证（强化版）─────────────────
            log(f"  操作前最终 COM 验证...", "WAIT")
            for pre_check in range(3):
                try:
                    _ = workbook.Application.Ready
                    _ = workbook.Sheets.Count
                    _ = workbook.Name
                    # 额外验证：访问具体单元格，确保 COM 真正可用
                    _ = workbook.Sheets(1).Range("A1").Value
                    log(f"  COM 最终验证通过", "OK")
                    break
                except Exception as pre_e:
                    log(f"  COM 最终验证第 {pre_check+1} 次失败: {pre_e}", "WARN")
                    if pre_check < 2:
                        time.sleep(5)
                        # 尝试刷新工作簿对象
                        ...
                        if fresh is not None:
                            ...
                            log(f"  刷新工作簿对象失败", "WARN")
                    else:
                        # 三次失败，抛出异常触发外层重试
                        raise Exception(f"COM 验证失败: {pre_e}")
            ...
            # ── 智能等待刷新完成（替代固定等待）────────────────
            refresh_ok = wait_for_refresh_complete(workbook)
            op_record["refresh_ok"] = refresh_ok
            op_record["refresh_elapsed_s"] = round(time.time() - refresh_start_time, 1)

            if not refresh_ok:
                log("  刷新超时，但仍尝试保存当前状态...", "WARN")
                op_record["error_detail"] += "refresh_timeout; "

            # ── 记录刷新后行数并对比 ──────────────────────────
            workbook = get_fresh_workbook(wb_name)
            if workbook:
                rows_after = get_used_row_count(workbook)
                ...
                log(f"  刷新后行数: {rows_after}", "INFO")
                if row_diff > 0:
                    log(f"  ✓ 新增 {row_diff} 行数据", "OK")
                elif row_diff == 0:
                    log(f"  ℹ 行数未变化（数据库与文件一致，无新数据）", "INFO")
                else:
                    log(f"  ⚠ 行数减少 {abs(row_diff)} 行（请检查数据）", "WARN")

                # 记录到 refresh_log.txt
                _save_refresh_log(wb_name, rows_before, rows_after, row_diff, refresh_ok)
                ...
                try:
                    ...
                    log(f"  保存失败: {save_err}", "ERROR")
                    op_record["save_ok"] = False
                    op_record["error_detail"] += f"save_failed: {save_err}; "
                    _save_formal_diagnosis_log(op_record)
                    return False
            else:
                op_record["error_detail"] += "get_fresh_workbook returned None; "
...
def close_workbook_safe(workbook, wb_name=None):
    """
    关闭工作簿（已 Save 过，此处 SaveChanges=False 避免弹窗）。
    锚定工作簿（用户预先打开的）不会被关闭。

    增强处理：
      1. 尝试通过 COM 对象直接关闭
      2. 若失败，通过文件名重新获取 COM 对象再试
      3. 若仍失败，通过 pywinauto 查找窗口并发送 Ctrl+F4 强制关闭
    """
    # 1. 获取工作簿名称
    name = None
    try:
    ...
    if not name and wb_name:
        name = wb_name

    if not name:
        log("  [关闭] 无法获取工作簿名称，跳过关闭", "WARN")
        return
    ...
    try:
        ...
        log(f"  通过 COM 关闭失败: {e}，尝试重新获取工作簿对象...", "WARN")
    ...
    if fresh_wb is not None:
        try:
            ...
            log(f"  重新获取后已关闭: {name}", "OK")
            time.sleep(1)
            return
        except Exception as e2:
            log(f"  重新获取后仍无法关闭: {e2}，尝试通过窗口强制关闭...", "WARN")
    else:
        log(f"  无法重新获取工作簿 [{name}]，尝试通过窗口强制关闭...", "WARN")

    # 4. 最后手段：通过 pywinauto 强制关闭窗口
    try:
        from pywinauto import Application
        from pywinauto.keyboard import send_keys
        # 查找标题包含文件名的 Excel 窗口
        ...
        win.set_focus()
        time.sleep(0.5)
        send_keys("^{F4}")  # Ctrl+F4 关闭当前工作簿
        log(f"  通过窗口强制关闭: {name}", "OK")
        time.sleep(1)
    except Exception as we:
        log(f"  通过窗口强制关闭失败: {we}", "ERROR")
...
def process_file_queue(file_queue):
    ...
    current_wb = None
    current_wb_name_prev = None   # 用于关闭时传递文件名
    is_first = True
    success_count = 0
    fail_count = 0
    ...
    for i, file_info in enumerate(file_queue):
        ...
        # 保存当前打开的文件名（用于后续关闭时传递）
        current_wb_name = filename   # 或从 new_wb.Name 获取，但文件名更可靠
        ...
        if current_wb is not None:
            log("  [先开后关] 关闭上一个文件...", "INFO")
            # 传入上一个文件的名称
            close_workbook_safe(current_wb, current_wb_name_prev)

        # 更新当前文件为下一个要关闭的对象
        current_wb = new_wb
        current_wb_name_prev = current_wb_name

    # 关闭最后一个文件
    if current_wb is not None:
        log("\n所有文件已处理完毕，关闭最后一个文件...", "STEP")
        close_workbook_safe(current_wb, current_wb_name_prev)
